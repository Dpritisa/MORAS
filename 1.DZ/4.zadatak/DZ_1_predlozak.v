...
Lemma pomocna: forall x y: list B,
NotL (AndL x y) = OrL (NotL x) (NotL y).
Proof.
intros z. induction z. reflexivity. destruct y. reflexivity.
simpl. rewrite IHz. destruct a,b;reflexivity. 
Qed.

Lemma pomocna2(x:list B):
NotL (NotL x)=x.
Proof.
induction x. simpl. reflexivity.
simpl. rewrite IHx. f_equal. destruct a. simpl. reflexivity. simpl. reflexivity.
Qed.

Lemma ALU_Or (x y : list B) : 
  length x = length y -> ALU x y O I O I O I = OrL x y.
Proof.
  intros. simpl. rewrite pomocna. rewrite pomocna2. rewrite pomocna2. reflexivity.
Qed.
 
Lemma ALU_One (n : nat) (x y : list B) :
  length x = n /\ length y = n /\ n <> 0 -> ALU x y I I I I I I = repeat O (pred n) ++ [I].
Proof.
 intros. simpl. destruct H. destruct H0. rewrite H. rewrite H0.
 destruct n. simpl. destruct H1. reflexivity. simpl. auto.
Abort.
